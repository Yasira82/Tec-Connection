// English is the SHAPE of every dictionary: `Dictionary = typeof en`, so a missing
// key in any other language is a TypeScript error rather than a blank space that
// only a speaker of that language would ever notice.
//
// `public.*` covers the three surfaces a stranger reaches with no session — the
// landing, /discover and /u/<handle>. Those are server-rendered, so they are also
// what a crawler and a link preview see; leaving them English-only made the front
// door unreadable for most of Pi's communities.
export const en = {
  common: {
    appName:  'TEC',
    tagline:  'The Elite Consortium',
    login:    'Sign in with Pi',
    logout:   'Logout',
    loading:  'Loading...',
    comingSoon: 'Coming Soon',
    live:     'Live',
    language: 'Language',
  },
  dashboard: {
    greeting:   'Welcome,',
    welcomeNew: '🎉 Welcome to TEC — Your account is ready',
    stats: {
      piBalance:     'Pi Balance',
      tecWallet:     'TEC Wallet',
      availableApps: 'Available Apps',
      activeApp:     'Active',
      subscription:  'Subscription',
      upgradePro:    'Upgrade to Pro',
    },
    appsTitle: 'TEC Ecosystem',
    appsCount: '24 Apps',
  },
  connection: {
    brand:       'TEC Connection · Your network',
    welcome:     'Welcome',
    welcomeName: 'Welcome, {name}',
    subtitle:    'your relationship graph in the TEC ecosystem',
    nav: { home: 'Home', discover: 'Discover', trust: 'Trust', settings: 'Settings',
      homeSub: 'What’s new, and who you follow.', discoverSub: 'Find people in the Pi economy.', trustSub: 'Built from completed Pi payments.' },
    footer:      'Connection is where your trusted relationships live — who you follow, connect with, and build trust with across TEC. You are always in control of your own network.',
    settings: {
      profile: 'Profile', planFree: 'Free', planPro: 'Pro',
      connectedPi: 'Connected to Pi', notSignedIn: 'Not signed in', member: 'TEC Member',
      appearance: 'Appearance', language: 'Language', languageDesc: 'Display language',
      theme: 'Theme', themeDesc: 'Light, dark, or follow your phone',
      themeSystem: 'System', themeLight: 'Light', themeDark: 'Dark',
      about: 'About', version: 'Version', domain: 'Domain', ecosystem: 'Ecosystem',
      builtOn: 'Built on', builtOnPi: 'Pi Network', logout: 'Logout',
    },
  },
  // Everything rendered INSIDE the authenticated app. Previously hardcoded
  // English in each component, so switching to Arabic produced a half-translated
  // screen: the headers changed, every card did not. A partly translated screen
  // is worse than an untranslated one — it reads as broken rather than as
  // unsupported.
  app: {
    notifications: 'Notifications',
    followedYou:   'started following you',
    noNotifications: 'Nothing yet.',

    connections:  'Connections',
    yourGraph:    'your social graph',
    followers:    'Followers',
    followingLbl: 'Following',
    followPlaceholder: 'Pi username — e.g. @alice',
    follow:       'Follow',
    unfollow:     'Unfollow',
    followingNow: 'Following ✓',
    noConnections: 'You are not following anyone yet.',

    searchPeople: 'Search people…',
    all:          'All',
    discoverEmpty: 'Nobody here yet. Publish your card in Settings to be the first.',
    discoverUnavailable: 'Discover is unavailable right now. Please try again shortly.',
    loading:      'Loading…',
    verifiedNote: '✅ Verified comes from Zone / KYC. ⭐ Featured is paid reach — never trust.',

    networkInsights: 'Network insights',
    proTag:       'PRO',
    proReveals:   'Pro shows you who they are, and lets you follow back in one tap.',
    proWhere:     'Settings → Connection Pro',
    notFollowedBack: 'Not followed back',
    mutual:       'Mutual ✓',
    onlineNow: 'Online now',
    onlineCount: 'online',
    followBack:   'Follow back',
    publishPrompt: 'Publish your profile in Settings so others can find you.',

    youPaid:   'You paid',
    paidYou:   'Paid you',
    sellers:   'sellers',
    buyers:    'buyers',
    partners:  'partners',
    orders:    'orders',
    seller:    'Seller',
    buyer:     'Buyer',
    trustEmpty: 'Nothing yet. This fills in from completed Pi payments across TEC.',
    trustRankTitle: 'Discovery ranking',
    trustRankBuilding: 'Counts toward your ranking in Explorer discovery — above businesses with no buyers. 5 buyers reaches the top tier.',
    trustRankTop: 'Top trust tier in Explorer discovery. Paid placement cannot outrank it.',

    collaboration: 'Collaboration',
    sharedCollections: 'shared collections',
    newCollection: 'New collection — e.g. Trip to Cairo',
    create:  'Create',
    owner:   'owner',
    member:  'member',
    items:   'items',
    members: 'members',
    backCollections: '← Collections',
    addItemPlaceholder: 'Add an item…',
    add: 'Add',
    noItems: 'No items yet.',
    invitePlaceholder: 'Invite a @username',
    invite: 'Invite',
    noCollections: 'No collections yet. Create one to plan or build together with your connections.',

    proTitle:    'Connection Pro',
    perMonth:    '/ month',
    proBenefit1: 'See who follows you — and follow back in one tap.',
    proBenefit2: 'A Featured card in Discover, so more people find you.',
    proReachOnly: 'Reach only — never verification or trust.',
    proUpgrade:  'Upgrade',
    proActive:   '★ You are on Pro',
    proExpires:  'Expires in {days} days',
    proSubActive: 'Your subscription is active. Thanks for supporting TEC.',
    proRenewNote: 're-subscribe to keep Pro (one-time monthly, no auto-renewal).',
    proPaid:      '✅ Connection Pro active',
    proPaidBody:  'Payment received. Thanks for supporting TEC Connection 🔗.',
    proPreparing: 'Preparing…',
    proConfirm:   'Confirm in Pi…',
    proCouldNotStart: 'Could not start the payment. Please sign in again and retry.',
    proFailed:    'Payment failed. Please try again.',
    proNotCompleted: 'Payment did not complete. Please try again.',

    yourCard:  'Your card',
    visibleInDiscover: 'Visible in Discover',
    notListed: 'Not listed',
    publicBadge: 'Public',
    hiddenBadge: 'Hidden',
    headlinePlaceholder: 'What do you do? (e.g. Pi app developer)',
    namePlaceholder: 'Your name (optional)',
    nameHint: 'Shown next to your Pi username.',
    save:      'Save',
    publish:   'Publish',
    hide:      'Hide',
    viewPublic: 'View your public page →',
    showOnline: "Show when I'm online",
    showOnlineOn: 'People in your groups and the people who follow you can see the green dot.',
    showOnlineOff: 'Hidden. Nobody sees you as online, anywhere.',
    showFollowers: "Show my follower count",
    showFollowersOn: "Visible on your public profile and on your Discover card.",
    showFollowersOff: "Hidden. Your profile shows everything else.",
    savedPublic: '✅ Saved — you are in Discover.',
    savedHidden: '✅ Saved — you are hidden.',

    addPhoto:   'Add a profile photo',
    photoTitle: 'Profile photo',
    photoHint:  'JPEG, PNG or WebP · up to 2MB. Shown publicly on your profile.',
    choosePhoto: 'Choose a photo',
    change:     'Change',
    remove:     'Remove',
    working:    'Working…',
    photoUpdated: '✅ Photo updated.',
    photoRemoved: 'Photo removed.',
    photoTooBig:  'That image is over 2MB. Try a smaller one.',
    photoWrongType: 'Use a JPEG, PNG or WebP image.',
    photoFailed: "The photo did not save. Please retry.",
    groupPhotoChange: "Change photo",
    groupPhotoRemove: "Remove",
    discoverGroups: "Find a group",
    searchGroupsPlaceholder: "Search groups by name",
    groupJoinNotice: "Joining needs the group owner's approval. You will see the group once they accept.",
    groupSearchFailed: "Could not load groups. Please retry.",
    groupNoResults: "No group matches that.",
    groupNoneListed: "No groups are listed yet.",
    groupNameTakenOwn: "You already have a group with that name. Try another.",
    groupNameTakenPublic: "A listed group already uses that name. Rename this one, then list it.",
    groupJoin: "Join",
    groupPending: "Requested",
    groupJoined: "Member",
    groupAskAgain: "Ask again",
    groupRequestSent: "Request sent. The owner will decide.",
    groupRequestFailed: "The request did not send. Please retry.",
    groupRequests: "Join requests",
    groupNoRequests: "No one is waiting.",
    groupApprove: "Approve",
    groupReject: "Reject",
    roleOwner: "Owner",
    roleAdmin: "Admin",
    makeAdmin: "Make admin",
    demoteAdmin: "Remove admin",
    removeMember: "Remove from group",
    silence: 'Silence',
    unsilence: 'Unsilence',
    silencedBadge: 'SILENCED',
    silenceHint: 'Stop this person posting here. They stay in the group and keep reading.',
    unsilenceHint: 'Let this person post here again.',
    youAreSilenced: 'You cannot post in this group right now.',
    confirmRemoveMember: "Tap again to remove",
    reply: "Reply",
    replyHint: "Answer this message, quoting it.",
    replyingTo: "Replying to",
    loadOlder: "Load older messages",
    quoteHidden: "You removed this message",
    searchMessages: "Search messages",
    searchMessagesPlaceholder: "Search in this chat",
    searchMessagesHint: "Type at least two letters.",
    searchMessagesFailed: "Search failed. Try again.",
    searchMessagesNone: "No messages match.",
    editMessage: "Edit",
    editMessageHint: "You can edit for 15 minutes after sending.",
    editingMessage: "Editing message",
    edited: "edited",
    forward: "Forward",
    forwardHint: "Send this message to another chat.",
    copyMessage: "Copy",
    copyMessageHint: "Copy the text of this message.",
    forwardTo: "Forward to",
    forwardSent: "Sent",
    forwardFailed: "Could not forward.",
    forwardNowhere: "You have no other chats yet.",
    forwardedFrom: "Forwarded from",
    pinMessage: "Pin",
    pinMessageHint: "Hold this at the top of the chat.",
    unpinMessage: "Unpin",
    unpinMessageHint: "Take it down from the top.",
    pinnedMessage: "Pinned",
    announcementOnly: "Only admins can post in this group.",
    announcementMode: "Only admins can post",
    announcementModeHint: "Members can read but not write. You can turn this off at any time.",
    inviteLink: "Invite link",
    inviteLinkHint: "Anyone with the link joins without asking. Re-creating it makes the old link stop working.",
    createInviteLink: "Create a link",
    revokeInviteLink: "Revoke the link",
    inviteLinkFailed: "Could not change the link. The group service may still be starting up — try again in a moment.",
    linkCopied: "Link copied",
    joiningByInvite: "Joining…",
    inviteInvalid: "That link is not valid any more.",
    inviteNeedsAuth: "Sign in with Pi to join — the invite is saved, and you will go straight in.",
    unreadDivider: "Unread messages",
    muted: "Muted",
    muteChat: "Mute this chat",
    unmuteChat: "Unmute this chat",
    muteHint: "Muted chats stop showing in the tab badge. You still get the messages, and the count stays on the row.",
    groupListed: "Listed publicly",
    groupListedHint: "Anyone can find this group and ask to join. Members and messages stay private.",
    groupPrivate: "Private",
    groupPrivateHint: "Only people you add can find this group.",
    groupDescriptionPlaceholder: "What is this group about? (optional)",
    uploadFailed: 'The upload did not complete. Please retry.',

    inviteTitle: 'Invite & earn',
    inviteSignedOut: 'Sign in with Pi to get your invite link.',
    inviteBody: 'Share your link — you both earn when a friend joins TEC through it.',
    copyLink: 'Copy link',
    copied: '✓ Copied',
    saveFailed:   'Could not save. Please retry.',
    networkError: 'Network error. Please retry.',

    messages: 'Messages',
    messagesSub: 'Your conversations.',
    noConversations: 'No conversations yet.',
    startConversation: 'Message someone from Discover, or start one below.',
    newMessage: 'New message — Pi username',
    messagePlaceholder: 'Write a message…',
    send: 'Send',
    newGroup: 'New group',
    groupTitlePlaceholder: 'Group name — e.g. Cairo builders',
    createGroup: 'Create',
    groupLabel: 'Group',
    directLabel: 'Direct',
    addMemberPlaceholder: 'Add a @username',
    addMember: 'Add',
    leaveGroup: 'Leave group',
    reportGroup: "Report this group",
    you: 'You',
    backToMessages: '← Messages',
    block: 'Block',
    unblock: 'Unblock',
    blocked: 'Blocked',
    blockedNotice: 'You have blocked this person. Unblock to message again.',
    messageRefused: 'Not delivered — either you are blocked, or you are sending too fast.',
    messageFailed: 'Could not send. Please retry.',
    threadUnavailable: 'This conversation is unavailable.',
    membersLabel: 'members',
    startChat: 'Start',
    newChat: 'New chat',
    onlyYouInGroup: 'You are the only one here. Add someone so your messages reach them.',
    groupInfo: 'Group info',
    today: 'Today',
    yesterday: 'Yesterday',
    noMessagesYet: 'No messages yet. Say hello.',
    searchOrType: 'Search a name, or type a Pi username',
    noPeopleFound: 'Nobody found with that name',
    messageExact: 'Message @{name} anyway',
    peopleYouFollow: 'People you follow',
    fromDiscover: 'From Discover',
    cancel: 'Cancel',
    openFailed: 'Could not open the chat ({code}). Please try again.',
    photo: 'Photo',
    photoUnavailable: 'Photo unavailable',
    voiceNote: 'Voice note',
    attachPhoto: 'Send a photo',
    recordVoice: 'Record a voice note',
    recording: 'Recording',
    stopRecording: 'Stop',
    micUnavailable: 'Voice notes are not available in this browser.',
    micDenied: 'Microphone access was refused.',
    micBlockedHere: 'Pi Browser did not allow the microphone. Voice notes need a browser that can.',
    contactInfo: 'Contact info',
    confirmLeave: 'Tap again to leave',
    playVoice: 'Play',
    pauseVoice: 'Pause',
    closeLabel: 'Close',
    clearChat: "Clear messages",
    confirmClear: "Tap again to clear",
    deleteChatHint: "Deleting erases your copy of this conversation for good. It is not a block — if they write again, the conversation returns empty.",
    myStatus: "My status",
    addStatus: "Add a status",
    statusHint: "Your status is visible for 24 hours to the people who follow you.",
    statusPlaceholder: "Write a status…",
    statusPhoto: "Photo",
    statusPost: "Post",
    statusFailed: "The status did not post. Please retry.",
    statusDelete: "Delete status",
    statusViews: "views",
    resetZoom: "Reset zoom",
    statusReplyPlaceholder: "Reply to this status…",
    statusReplySent: "Reply sent to your chat",
    statusReplyToPhoto: "replying to your photo",
    statusNext: "Next",
    statusPrevious: "Previous",
    justNow: "just now",
    minutesAgo: "{n}m ago",
    hoursAgo: "{n}h ago",
    report: "Report",
    reportHint: "Reviewed by a person. Reporting does not remove anything on its own.",
    reportNotice: "A person reviews every report. Reporting does not remove the content by itself — to stop contact now, block instead.",
    reportSubmit: "Send report",
    reportSent: "Report sent",
    reportSentHint: "Someone will review it. The content is not removed automatically — to stop contact now, block instead.",
    reportFailed: "The report did not send. Please retry.",
    reportNotePlaceholder: "Details (optional)",
    reason_spam: "Spam or advertising",
    reason_scam: "Scam or fraud",
    reason_harassment: "Harassment or abuse",
    reason_sexual: "Sexual content",
    reason_violence: "Violence or threats",
    reason_other: "Something else",
    reportedBy: "Reported by",
    modQueue: "Reports",
    modEmpty: "Nothing to review.",
    modTab_OPEN: "Open",
    modTab_REVIEWED: "Reviewed",
    modTab_ACTIONED: "Actioned",
    modTab_DISMISSED: "Dismissed",
    modActioned: "Action taken",
    modReviewed: "Mark reviewed",
    modDismissed: "Dismiss",
    attachTooBig: 'That file is too large.',
    attachFailed: 'The attachment did not send. Please retry.',
    sending: 'Sending…',
    message: 'Message',
    blockedList: 'Blocked',
    noBlocked: 'No one blocked.',
    confirmBlock: 'Tap again to block',
    blockFailed: 'Could not change the block. Please retry.',
    deleteMessage: 'Delete',
    deleteForMe: "Delete for me",
    deleteForMeHint: "Removed from your copy only. The other person still sees it.",
    deleteForEveryone: "Delete for everyone",
    deleteForEveryoneHint: "Cleared on both sides, replaced by “This message was deleted”.",
    messageDeleted: 'This message was deleted',
    deleteChat: 'Delete conversation',
    confirmDelete: 'Tap again to delete',
    typingNow: 'typing…',
    seen: 'Seen',
    delivered: 'Sent',
    newMessageFrom: 'sent you a message',
  },
  public: {
    brand:        'TEC · Connection',
    headline:     'The people of the Pi economy.',
    lede:         'Find builders and merchants who actually accept Pi — and see who is trusted, from real completed payments rather than claims.',
    alreadyOn:    'already on TEC',
    cta:          'Continue with Pi',
    browseFree:   'Or browse without signing in →',
    peopleOnTec:  'People on TEC',
    seeAll:       'See all →',
    howTrust:     'How trust works here',
    step1Title:   'Publish',
    step1Body:    'Put up a handle and one line about what you do. Opt-in — you are listed only if you choose to be.',
    step2Title:   'Connect',
    step2Body:    'Follow the builders and merchants you actually deal with. Your graph belongs to you.',
    step3Title:   'Be trusted',
    step3Body:    'Trust here is computed from completed Pi payments — not from followers, reviews, or anything you can write about yourself.',
    disclaimer:   'Verification is presented from Zone / KYC — never minted by Connection. Featured is a Pro placement: reach only, not trust.',

    discoverTitle: 'Discover people on Pi',
    discoverLede:  'Builders, merchants, creators and investors. Verification comes from Zone / KYC — Connection presents it, never mints it.',
    searchLabel:   'Search by handle or headline',
    searchHint:    'Search people…',
    searchAction:  'Search',
    all:           'all',
    emptyFiltered: 'Nobody matches that yet',
    emptyAll:      'The directory is still filling up',
    emptyBody:     'Listing is opt-in — people appear here only after publishing their profile. Sign in and publish yours to be found.',
    clearFilters:  'Clear filters',
    findableTitle: 'Be findable in the Pi economy',
    findableBody:  'Publish your profile, follow the people you deal with, and let trust build from real completed payments.',

    back:          'Discover',
    verified:      'Verified',
    verifiedHint:  'Verified — presented from Zone / KYC, never minted by Connection',
    featured:      'Featured',
    featuredHint:  'Featured — a Connection Pro placement. Reach only; not verification.',
    follower:      'follower',
    followers:     'followers',
    since:         'On TEC since {date}',
    follow:        'Follow @{name}',
    following: "Following @{name}",
    followSignIn: "Follow @{name} with Pi",
    followSelf: "This is your profile",
    followFailed: "Could not follow. Try again.",
    shareProfile: "Share this profile",
    shareCopied: "Link copied",
    shareText: "@{name} on TEC — the people of the Pi economy.",
    browseMore:    'Browse more people →',
    profileNote:   'Verification is presented from Zone / KYC — never minted by Connection. Featured is a Connection Pro placement: reach only, never trust. Trust is earned, never bought.',
    notFound:      'Profile not found',

    cat: {
      builder:  'builder',
      merchant: 'merchant',
      creator:  'creator',
      investor: 'investor',
      mentor:   'mentor',
      other:    'other',
    },

    privacy: 'Privacy',
    terms:   'Terms',
  },
};

export type Dictionary = typeof en;
