...
function Snapshot({ raw }: { raw: string }) {
  ...
  return (
    <div style={{
      ...
      fontSize: 12.5, color: C.text, lineHeight: 1.55, wordBreak: 'break-word',
    }} dir="auto">
      {text || <span style={{ color: C.subtext }}>{t.app.photo}</span>}
      {text && media ? <span style={{ color: C.subtext }}> · {t.app.photo}</span> : null}
    </div>
  );
}
    ...
    <div style={{ padding: '12px 14px', borderTop: `1px solid ${C.border}` }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        <span style={{
          ...
          color: C.gold, background: goldA(0.078),
          border: `1px solid ${goldA(0.2)}`, borderRadius: 999, padding: '2px 9px',
        }}>{(a[`reason_${r.reason}` as keyof typeof a] as string) ?? r.reason}</span>
        <span style={{ fontSize: 11.5, color: C.subtext }}>{r.kind}</span>
        <span style={{ flex: 1 }} />
      ...
      <p style={{ margin: '6px 0 0', fontSize: 12, color: C.subtext }}>
        {a.reportedBy} <bdi>@{r.reporter_username}</bdi>
      </p>

      {r.note && (
        <p style={{ margin: '6px 0 0', fontSize: 13, color: C.text, lineHeight: 1.55 }} dir="auto">
          {r.note}
        </p>
      )}
      ...
      {r.status === 'OPEN' ? (
        <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginTop: 10 }}>
          <button
            ...
            style={{
              ...
              color: C.error, borderRadius: 999, padding: '7px 15px',
        ...
        <p style={{ margin: '8px 0 0', fontSize: 11.5, color: C.subtext }} dir="auto">
  ...
  return (
    <section style={{ marginTop: 20 }}>
      ...
      <div style={{
        ...
        {loading ? (
          <div style={{ padding: '14px 16px', fontSize: 13, color: C.subtext }}>{a.loading}</div>
        ) : groups.length === 0 ? (
          <div style={{ padding: '14px 16px', fontSize: 13, color: C.subtext }}>{a.modEmpty}</div>
        ) : groups.map((g) => (
          <div key={g.author} style={{ borderTop: `1px solid ${C.border}` }}>
            <div style={{
              ...
              <span style={{
                ...
                color: g.count > 1 ? C.error : C.subtext,
                background: g.count > 1 ? errorA(0.078) : 'transparent',
                border: `1px solid ${g.count > 1 ? errorA(0.267) : C.border}`,
              }}>{g.count}</span>
            </div>
            {g.reports.map((r) => (
